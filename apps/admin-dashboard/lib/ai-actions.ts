 ...
 *
 * This is called when a user visits the /analytics/apps/<appClass> page.
 * It checks the App record first:
...
export async function suggestAppName(
  ...
  // If user already renamed the app, or already has a suggestion (accepted or dismissed), skip AI
  ...
  const { object } = await generateObjectWithTelemetry({
    model,
    schema: z.object({
      shouldSuggest: z.boolean().describe("Whether a rename suggestion would be helpful"),
      suggestedName: z.string().nullable().describe("The suggested human-friendly display name, or null if the current name is already clear"),
    }),
...
${topWindowTitles.slice(0, 6).map((title, i) => `${i + 1}. "${title}"`).join("\n")}
...
If a better name exists, set shouldSuggest=true and provide a short, recognisable display name. Use the window titles as clues to figure out what the app actually is. Keep the name concise — usually 1-3 words (e.g. "Google Chrome", "VS Code", "Files", "Terminal", "LibreOffice").`,
  ...
  // AI says the name is fine — record that we checked so we don't ask again
  await db.app.update({
    where: { appClass },
...
export async function createProjectFromRescueTime(rtProjectId: number): Promise<{
  ...
  // Fetch the RescueTime project with its aggregated time stats
  ...
  // Gather a sample of comments for context
  const sampleComments = rtProject.projectTimes
    .filter((t) => t.comment)
    .slice(0, 20)
    .map((t) => t.comment as string);

  // Fetch all existing clients so AI can match
  ...
  const projectSchema = z.object({
    ...
    clientId: z
      .number()
      .describe("ID of the best-matching existing client from the provided list"),
